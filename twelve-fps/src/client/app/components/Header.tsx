...
export const Header: React.FC<HeaderProps> = ({ currentView, setCurrentView }) => {
  ...
  return (
    ...
    >
        ...
        {navigation.map(({ key, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`group/item relative flex items-center justify-center rounded-md mx-1 py-2 text-[11px] font-medium border transition outline-none focus:ring-2 focus:ring-white/50 ${active ? 'bg-white/25 border-white/60 text-white shadow-inner' : 'bg-white/10 border-white/20 text-white/70 hover:bg-white/20 hover:text-white'}`}
              aria-label={label}
            >
      ...
      <div className="py-3 text-[10px] text-white/30 border-t border-white/10 font-mono tracking-wide flex items-center justify-center">
