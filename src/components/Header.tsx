...
export const Header: React.FC<HeaderProps> = ({ currentView, setCurrentView }) => {
  ...
  return (
    <aside className="fixed top-0 left-0 h-screen w-48 flex flex-col bg-white/10 backdrop-blur-xl border-r border-white/20 shadow-xl z-40">
      <div className="flex items-center gap-2 px-4 py-4 select-none border-b border-white/10">
        ...
        {navigation.map(({ key, label, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`group w-full flex items-center gap-2 rounded-md px-3 py-2 text-sm font-medium border transition outline-none focus:ring-2 focus:ring-white/50 ${active ? 'bg-white/25 border-white/60 text-white shadow-inner' : 'bg-white/10 border-white/20 text-white/70 hover:bg-white/20 hover:text-white'}`}
            >
      ...
      <div className="px-3 py-3 text-[10px] text-white/40 border-t border-white/10 font-mono tracking-wide">
        v0.1 session
      </div>
    </aside>
  );
